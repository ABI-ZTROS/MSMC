...
const STEPS: TutorialStep[] = [
  ...
  },
  {
    ...
    lines: [
      ...
      '跑一次体检，它会检查 Java 版本、内存、端口、防火墙、日志里的 OOM……一项一项告诉你哪里有问题 🧐',
      '想让 AI 帮你总结诊断结论？先给它"发工资"：在「疑难解答」页面上方找到「DeepSeek AI 诊断助手」卡片，把我下面教的 API Key 粘贴进去并点「保存 Key」。',
      '怎么拿到 Key？去 platform.deepseek.com 注册登录 → 左边「API Keys」→ 新建一个 → 把那个 sk- 开头的一长串复制过来。跟着点，一分钟搞定，比开服简单多了 😏',
      '配置好 Key 后，体检报告会自动附上 AI 诊断结论；搞不明白还能在下面的输入框向 AI 追问，比如"这内存够不够？为啥老崩？"',
      '查出毛病还能直接点「一键修复」——它会先帮你备份，再动手，改坏了也能回滚。这就是"医生先化验再开药"🤓',
      '要是它说你没问题你却觉得卡：多半是网络/电脑配置的事，别甩锅给服务器 😂',
    ],
    action: '疑难解答 → 顶部粘贴 DeepSeek API Key → 保存 → 跑体检 → 看 AI 结论',
  },
  {
    icon: '🛡️',
  ...
  },
  {
    ...
    lines: [
      ...
      '万一看体检报告还是懵？去「疑难解答」页面上方配好 DeepSeek API Key（第七步教的），然后就能向 AI 追问了——把你的报错贴给它，它比搜索引擎懂你多了 🤓',
    ],
    action: '遇到问题 → 疑难解答 → 已配 Key 就追问 AI，没配 Key 先看第七步',
  },
]
